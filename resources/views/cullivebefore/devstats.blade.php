...
<div class="row">
  <div class="col-md-8">
    @include(config('cullivebefore.mainrouter').'.devstats.devlist')
    <div class="box box-default">
      ...
      <div class="box-body">
        <div class="alert alert-info alert-dismissible">
          ...
          <span>共用 {{ $pagetag->getAllcols() }} 个设备，其中 监测类 {{ App\Model\Device::where('attr', 1)->count() }}，控制类 {{ App\Model\Device::where('attr', 2)->count() }}</span>
        </div>
        <div class="alert alert-warning alert-dismissible">
          <h4><i class="icon fa fa-warning"></i> 状态警告</h4>
          <span>有 {{ App\Model\Device::where('attr', '!=', 3)->where('data', null)->count() }} 个设备处于无数据状态</span>
        </div>
      </div>
    </div>
  </div>
