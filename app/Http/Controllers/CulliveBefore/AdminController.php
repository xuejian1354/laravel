...
class AdminController extends Controller
{
	public function index(Request $request) {
		...
		if($request->isMethod('get')) {
			return redirect(config('cullivebefore.mainrouter').'/curinfo');
		}
	}
		...
		elseif($curopt == 'user') {
			...
			if($request->isMethod('post')) {
				if($request->input('way') == 'userlist' || $request->input('way') == 'userdel') {
					return view(config('cullivebefore.mainrouter').'.curinfo.userlist')
	...
	public function areaCtrl(Request $request, $areasn = null, $areaopt = null) {
		...
		if($request->isMethod('post')) {
			...
			if(strstr($waystr, 'devlist') !== false) {
				...
				return view(config('cullivebefore.mainrouter').'.areactrl.devlist')
						->with('listid', $listid)
						->with('request', $request)
						->with($pagewithdevs);
			}
	...
	public function devStats(Request $request, $devopt = null) {
		...
		if($request->isMethod('post')) {
			if($request->input('way') == 'devlist') {
				return view(config('cullivebefore.mainrouter').'.devstats.devlist')
				->with('request', $request)
				->with('devtypes', Devtype::all())
				->with('areas', Area::all())
	...
	public function videoReal(Request $request, $camopt = null) {
		...
		if($request->isMethod('post')) {
			if($request->input('way') == 'videolist') {
				return view(config('cullivebefore.mainrouter').'.videoreal.videolist')
						->with('request', $request)
						->with('pagetag', $pagetag)
						->with($vedfiles);
			}
	...
	public function alarmInfo(Request $request) {
		...
		if($request->isMethod('post')) {
			if($request->input('way') == 'alarmlist') {
				return view(config('cullivebefore.mainrouter').'.alarminfo.alarmlist')
						->with('request', $request)
				...
				return view(config('cullivebefore.mainrouter').'.alarminfo.msglist')
	...
	{
	    $expth = explode('/', $request->path(), 2);
		$select_menus = array();
		$console_menus = (new ConsoleMenu())->getConsleMenuLists(end($expth));
		...
		return view(config('cullivebefore.mainrouter').'.'.$view, $data, $mergeData)
				->with('request', $request)
	...
	public static function withurl($url) {
		...
		return url(config('cullivebefore.mainrouter').'/'.$url);
	}
}
